        ...
        <div className="row g-0 ">
          ...
          <div className="col-12 col-md-9">
            <div className="card-body m-3">
              <h1 className="display-6 fw-bold card-title">{movie.title}</h1>
              ...
              <div className="w-100 d-flex mt-4 flex-wrap">
                {movie.genres.map((item) => (
                  <p key={item.id} className="badge btn-movies ms-2">{item.name}</p>
                ))}
              </div>

              <div className="d-flex w-100 justify-content-start">
                <p className="ps-2 small text-muted fst-italic">
                  <span className="fw-bold">Duration</span>: {movie.runtime} min
                </p>
                <p className="ps-5 small text-muted fst-italic">
                  <span className="fw-bold">Language</span>: {movie.spoken_languages.map((item) => item.name).join(", ")}
                </p>
              </div>
              ...
              <div className="mt-3 d-flex justify-content-between w-100">
                <img
                  ...
                  style={{ height: '50px', width: '90px', borderRadius: '20px' }}
                  alt="company logo"
                />

                <a className="ps-2" href={movie.homepage} target="_blank" rel="noopener noreferrer">
                  <button className="btn btn-movies-outline text-white">
